...
// Only validate completion if both dates are filled
if (joinVal && compVal && new Date(compVal) <= new Date(joinVal)) {
  setError(fields.complete, 'Completion must be after joining');
  ok = false;
} else {
  setError(fields.join, '');
  setError(fields.complete, '');
}

// History (optional) → no required check
setError(fields.history, '');
