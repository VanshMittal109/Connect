...
function validate() {
  ...
  if (!ageVal || ageVal < 1 || !Number.isInteger(ageVal)) { setError(fields.age, 'Enter a valid positive age'); ok = false; }
  else setError(fields.age, '');
  ...
  const compVal = fields.complete.value;
  if (!joinVal) { setError(fields.join, 'Joining date is required'); ok = false; } else setError(fields.join, '');
  if (!compVal) { setError(fields.complete, 'Completion date is required'); ok = false; }
  else if (joinVal && new Date(compVal) <= new Date(joinVal)) { setError(fields.complete, 'Completion must be after joining'); ok = false; }
  else setError(fields.complete, '');

  // History
  const histVal = fields.history.value.trim();
  if (!histVal) { setError(fields.history, 'Medical history is required'); ok = false; }
  else setError(fields.history, '');
...
document.getElementById('patient-table')?.addEventListener('click', (e) => {
  ...
  if (btn.classList.contains('delete')) {
    ...
    fields.age.value = tds[2].textContent;
    ...
    fields.complete.value = tds[7].textContent;
